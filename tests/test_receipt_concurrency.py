...
async def test_concurrent_duplicate_receipts_create_one_event() -> None:
    ...
    try:
        async with httpx.AsyncClient(
            ...
            assert (
                completed_events[0]["fromStatus"]
                == "PROCESSING"
            )
            assert (
                completed_events[0]["toStatus"]
                == "COMPLETED"
            )

    finally:
        try:
            # Отменяем retry, который мог запустить worker,
            # пока provider-simulator был остановлен.
            await run_compose(
                "restart",
                "candidate-service",
            )
